...
write.csv(bind_cols(snu.replicates.estrogen2,snu.replicates.creatinine2), file = "data/qc_data/snu.qc.csv",row.names=FALSE, na="")
```
...
control.estrogen = df.batch %>% 
	...
	select(-injection) %>% filter(batch.num != 8)

# creatinine controls
control.creatinine= df.creatinine%>% 
	filter((str_detect(customer.id, regex("control",ignore_case=TRUE))))

write.csv(full_join(control.estrogen,control.creatinine,by=c("customer.id")), file = "data/qc_data/control.qc.csv",row.names=FALSE, na="")
```
